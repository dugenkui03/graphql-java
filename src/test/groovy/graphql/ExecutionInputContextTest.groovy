...
class ExecutionInputContextTest extends Specification {

    def buildContext(Map<String, String> map) {
        def context = ExecutionInputContext.newContext()
        map.forEach({ k, v -> context.of(k, v) })
        return context.build()
    }

    int sizeOf(ExecutionInputContext graphQLContext) {
    ...
    def "of builder"() {
        def context
        when:
        context = ExecutionInputContext.newContext().of("k1", "v1").build()
        then:
        context.get("k1") == "v1"
        sizeOf(context) == 1

        when:
        context = ExecutionInputContext.newContext().of(
                "k1", "v1",
                "k2", "v2"
        ).build()
        then:
    ...
    def "graphql context integration test"() {
        ...
        DataFetcher df = { env ->
            ExecutionInputContext context = env.context
            return context.get("ctx1")
        }
        def graphQL = TestUtil.graphQL(spec, ["Query": ["field": df]]).build()

        def context = ExecutionInputContext.newContext().of("ctx1", "ctx1value").build()
