...
class AsyncUtilTest extends Specification {

    def "eachSequentially test"() {
        given:
        def input = ['a', 'b', 'c']
        def cfFactory = Mock(AsyncUtil.CFFactory)
        def cf1 = new CompletableFuture()
        def cf2 = new CompletableFuture()
        def cf3 = new CompletableFuture()

        when:
        def result = AsyncUtil.eachSequentially(input, cfFactory)
    ...
    def "each works for mapping function"() {
        ...
        when:
        def result = AsyncUtil.each(input, cfFactory)
    ...
    def "each works for list of futures "() {
        ...
        when:
        def result = AsyncUtil.each([completedFuture('x'), completedFuture('y'), completedFuture('z')])
