...
class IDLDocumentParserTest extends Specification {
...
"""
        ...
        when:
        def document = new DocumentParser().parseDocument(input)
...
"""
    ...
    def "object type extensions"() {
        ...
        when:
        def doc = new DocumentParser().parseDocument(input)
    ...
    def "source name is available when specified"() {
        ...
        when:
        def defaultDoc = new DocumentParser().parseDocument(input)
        def namedDocNull = new DocumentParser().parseDocument(input, null)
        def namedDoc = new DocumentParser().parseDocument(input, sourceName)
